...
FUNCTION MAIN()
  ...
  DEFINE opts sDAdyn.T_SingleTableDAOptions =
    (sqlAll: "SELECT * FROM CUSTOMER AS cust, cust_ex WHERE cust.customer_num=cust_ex.customer_num",
      browseForm: "cust_ex",
      browseRecord: "scr",
      sqlFetchByPosition: TRUE, --must use fetch by position because of the multiple names
      qualifiedNames:TRUE, --must use qualified names to avoid a run time error (multiple column names)
