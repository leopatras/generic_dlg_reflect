...
MAIN
  ...
  END FOR
  {
  SELECT COUNT( * ) INTO n1 FROM customer
  DISPLAY  "count customers:"  ,n1
  SELECT COUNT( * ) INTO n2 FROM orders
  DISPLAY  "count orders:"  ,n2
  SELECT COUNT( * ) INTO n3 FROM items
  DISPLAY  "count items:"  ,n3
  }
  FOR i=1 TO cu.getLength()
...
FUNCTION fetch_orders_by_cust(orders T_orders,customer_num LIKE customer.customer_num)
  DEFINE order T_order
  DEFINE n INT
  DECLARE cu2 CURSOR FOR SELECT * INTO order.* FROM orders  WHERE customer_num = @orders.customer_num
  FOREACH cu2
     LET n = n + 1
...
FUNCTION fetch_items_by_order(items T_items,order_num LIKE orders.order_num)
  DEFINE item T_item
  DEFINE n INT
  DECLARE cu3 CURSOR FOR SELECT * INTO item.* FROM items  WHERE order_num = @items.order_num
  FOREACH cu3
     LET n = n + 1
     LET items[n].* = item.*
  END FOREACH
